...
function _write_daily_climate(path, variable_name, units, values)
...
@testset "Monthly nitrogen deposition climate forcing" begin
    mktempdir() do directory
        temp = _write_daily_climate(joinpath(directory, "temp.nc"), "temp", "degC", fill(10.0f0, 365))
        prec = _write_daily_climate(joinpath(directory, "prec.nc"), "prec", "mm/day", zeros(Float32, 365))
        lwnet = _write_daily_climate(joinpath(directory, "lwnet.nc"), "lwnet", "W/m2", fill(-50.0f0, 365))
        swdown = _write_daily_climate(joinpath(directory, "swdown.nc"), "swdown", "W/m2", fill(100.0f0, 365))
        no3 = _write_monthly_deposition(joinpath(directory, "no3.nc"), "noy", Float32.(1:12))
        nh4 = _write_monthly_deposition(joinpath(directory, "nh4.nc"), "nhx", Float32.(2:2:24))
        co2 = joinpath(directory, "co2.txt")
        write(co2, "2015 400.0\n")
