...
@kernel inbounds = true function cultivate_kernel!(
    ...
    window_assimilate::AbstractVector{T},
    ...
    if sowing
        ...
        # A new season starts with full grain-set and grain-filling potential.
        # Without this the irreversible damage of one season would carry into
        # the next.
        grain_set_fraction[cell] = one(T)
        grain_fill_fraction[cell] = one(T)
        heavy_rain_excess[cell] = zero(T)
        lodging_exposure[cell] = zero(T)
