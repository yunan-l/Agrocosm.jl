...
@testset "Prescribed manure is split and conserved" begin
    ...
    fertilizer!(crop_cal, managed_land, crop, soil, 1; manure = true)
    @test crop.nitrogen.pending_manure[1] ≈ 8.0f0
    @test crop.nitrogen.prescribed_manure_input[1] ≈ 2.0f0

    crop.phenology.fphu .= 0.3f0
    fertilizer!(crop_cal, managed_land, crop, soil, 2; manure = true)
    @test crop.nitrogen.prescribed_manure_input[1] ≈ 8.0f0
    ...
    @test crop.nitrogen.pending_manure[1] == 0.0f0
end
