...
@testset "LPJ CO2 units and minimum conductance" begin
    ...
    crop.carbon.root .= 1000.0f0
    crop.water.root_distribution .= 0.0f0
    crop.water.root_distribution[1] = 1.0f0
    crop.canopy.fpar .= 0.8f0
    crop.water.canopy_wet .= 0.0f0
    pet.eeq .= 1.0f0
    ...
    expected_gp = 1.6f0 * adtmm[1] /
                  ...
                  cft1.gmin * crop.canopy.fpar[1]

    transpiration!(adtmm, cft1, crop, pet, soil, co2_pa)

    @test crop.water.canopy_conductance[1] ≈ expected_gp rtol = 1.0f-6
    @test crop.water.canopy_conductance[1] > cft1.gmin * crop.canopy.fpar[1]
end
...
@testset "Inactive crop has zero conductance" begin
    ...
    @test crop.water.canopy_conductance[1] == 0.0f0
end
